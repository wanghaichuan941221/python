...
class Mapping:
    ...
    def update_map(self, grid_map, pose, scan):
        ...
        ox,oy=[],[]
        obs = []
        start = [int((position.x-origin.position.x)/resolution),int((position.y - origin.position.y)/resolution)]
        ...
        for index in range(0,len(ranges)):
            ...
            if ranges[index]>range_min and ranges[index]<range_max:
                ...
                for index1 in range(len(traversed)):
                    self.add_to_map(grid_map,traversed[index1][0],traversed[index1][1],self.free_space)
                    ox.append(traversed[index1][0])
                    oy.append(traversed[index1][1])
                    
        for point in obs:
            self.add_to_map(grid_map,point[0],point[1],self.occupied_space)
        ...
        update.width = int(max(ox) - update.x + 1)
        # Maximum y index - minimum y index + 1
        update.height = int(max(oy) - update.y + 1)
